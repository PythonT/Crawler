...
#import urllib2
import urllib.request #this is the old urllib2
...
headers2 = {"Host": "www.google.com","User-Agent": "Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.1) Gecko/20090624 Firefox/3.5","Accept": "text/plain"}
...
def saveHtmlSrc(url):
    ...
    f = open('jd_iphone.txt','wb')
    #str(html).encode('gbk', errors='replace').decode('gbk')
    f.write(html)
    f.close()
...
fhtml = open('jd_iphone.txt','r',encoding='utf-8')#Read file use utf-8
...
for sku in re.findall('<lisku="\d+">', localhtml):
    ...
    if len(pricehtmlSrc) > 5:
        #pricehtmlSrc = str.decode(pricehtmlSrc)
        pprice=json.loads(pricehtmlSrc.decode())
        print(pprice[0]['p'])
    else:
